...
#---------------------------
def calculate_dcmd(Ts, Tp, massins, massinp, ws):
#---------------------------
        ...
        salinity=ws # g/kg salinity in the feed
        #---------------------------
        ...
        mfin = massins
        mpin = massinp
        vin = massins/(1000.0*dx*B)
        vinp = massinp/(1000.0*dx*B)
        Res = 1000.0*vin*Dh/Mus
        Rep = 1000.0*vinp*Dh/Mup
        ...
        per_out=(rr_dcmd*mfin)+mpin
        tperout=(1.0/per_out)*((Q_dcmd/Cp)+(mfin*Tp))
        return rr_dcmd
#---------------------------
# Calculation for AGMD
#---------------------------
def calculate_agmd(Ts, Tp, massins, massinp, ws):
        L=0.07 # Length of the module 
        dx=0.022 # Flow channel width 
        B=0.006 # Height of the module
        dm=0.000160 # m Thickness of membrane 
        ...
        permeability = calculate_diffusivity(T_agmd[1],T_agmd[2],da)
        activity_factor = calculate_activity_factor(salinity)
...
print calculate_agmd(40, 30, 0.05, 0.05, 0.035)
# #---------------------------
